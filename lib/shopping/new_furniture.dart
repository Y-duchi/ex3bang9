...
class NewFurniturePage extends StatefulWidget {
  const NewFurniturePage({super.key});

  @override
  State<NewFurniturePage> createState() => _NewFurniturePageState();
}

class _NewFurniturePageState extends State<NewFurniturePage> {
  List furnitureList = [];
  List<bool> likedList = [];
  ...
  Future<void> fetchFurnitureList() async {
    ...
    if (response.statusCode == 200) {
      ...
      // ✅ created_at 없는 항목 제거
      furnitureList.removeWhere((item) => item['created_at'] == null);

      // ✅ 최신순 정렬
      furnitureList.sort((a, b) =>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('최신 가구', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
